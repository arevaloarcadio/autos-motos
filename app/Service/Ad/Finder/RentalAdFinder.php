 ...
 */
class RentalAdFinder implements IAdFinder
{
    ...
    private function applyFilters(Builder $query, array $input): Builder
    {
        $query = $query
            ->with(['rentalAd'])
            ->select('ads.*')
            ->join('rental_ads as ra', 'ra.ad_id', '=', 'ads.id');
        ...
        if (isset($input['city'])) {
            $query->where('ra.city', 'LIKE', sprintf('%%%s%%', $input['city']));
        }
    ...
    {
        return view('rental-ad.rental-ad-list', compact('ads', 'adType', 'adSubType'));
    }
}
